...
Druhá intervence měla za cíl navrhnout postup pro realizaci syntetické analytické zprávy pro podporu designu nových koncepčních prvků v RIS3. Konkrétně se jedná o zapojení veřejnosti do EDP procesu, kde bylo zapotřebí zmapovat překážky pro jejich zapojení a navrhnout způsoby jejich řešení.

### Uživatelské testování webového produktu

Řada analytických výstupů se prezentuje na webových kanálech. V zájmu posílení jejich využitelnosti a přehlednosti jsme navrhli postup, jak testovat webový produkt přímo s uživateli pomocí metod uživatelského designu (UX design).
...
- využívání evropských sítí a expertizy (JRC) - viz paper k misím v RIS3
- využívání a budování specializované expertizy (TC) - zkrz rámcovku pěstujou to TC (lepší než banda kozultaček)
- spolupráce v tématu inovací napříč organizacemi (CzechInvest, TA ČR, TC - datový tým), vč. datových -  (TODO prolink na systémovou tezi o klastrech)

# Zobecnitelnější vhledy

V oddělení S3 se koncentrují všechny činnosti spojené se strategickou prací, od koncepce po koordinaci, ale také poskytování dat a podkladů do celého prostředí a síťování všech aktérů.

Přestože se nejedná o čistě analytický útvar, oddělení S3 má vybudovanou odbornou kapacitu v tématu podpory vědy a výzkumu a vytváří datové produkty pro celé prostředí. 

Know-how pro práci s daty je koncentrovaná v jednom až dvou členech týmu, což je relativně běžné i v jiných analytických útvarech (např. na ASZ). To přináší výhodu z hlediska budování know-how a zajištění kontinuity, nicméně se tím zvyšuje i závislost na těchto členech týmu. Specificky v oblasti dat vzniká také riziko, že postupy pro správu dat (data management) nejsou zexplicitněny a aplikovány napříč týmem.  

Během projektu se ukázalo, že v některých případech může být užitečné vyhledat expertizu i mimo veřejnou správu, zejména pro aplikaci technických postupů.
...
Aktivity na podporu implementace strategie (vč. analytické práce) jsou téměř výhradně hrazené skrz evropské fondy a projektoké financování. V důsledku jsou tyto aktivity rozvíjeny v krátkých časových rámcích typické pro projektové financování (2-3 roky), což ohrozuje jejich kontinuitu a zvyšuje nejistotu členů týmu ohledně jejich budoucího působení.  
